...
// Bodog.com Hand #3890130788 TBL#20642234 HOLDEM No Limit - 2020-02-18 08:53:26
// It appears that Bovada/Bodog has an identical history format
const roomGameID =
  // Ignition Hand #3548320887:
  '^(Ignition|Bovada|Bodog.com) (?:Hand|Game) #(\\d+):? +'
  ...
  _gameType() {
    ...
    for (var i = 0; i < lines.length && lines[i].length; i++) {
      ...
      if (cashGameInfo.test(lines[i])) {
        ...
        this._tableFast = lines[i].search('Zone') > 0
        return this._cachedGameType
      }
    }
    return null
