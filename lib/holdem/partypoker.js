...
const {
  ...
  , limit
  , card
} = require('../util/rx-partials')

const { fixAllinActionsForAll } = require('../util/fix-allin-actions')
    ...
    for (var lineno = 0; lineno < this._lines.length; lineno++) {
      ...
      if (this._section === section.SETUP) {
        ...
        if (this._item === item.GAMEID) {
          ...
          throw new Error(`Expected preflop indicator but found:\n  ${line}`)
        }
...
function canParse(lines) {
  const fl = lines[0]
  return gameIDRx.test(fl) || preludeGameNoRx.test(fl) || preludeGameStartRx.test(fl)
}
