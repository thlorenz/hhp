...
proto._readInfo = function _readInfo(line, lineno) {
  ...
  const handInfoRx = this._handInfoRx(gameType)
  const match      = line.match(handInfoRx)
  if (!match) return

  const donation = safeParseFloat(match[5])
  const rake     = safeParseFloat(match[7])
  ...
  this.hand.info = {
    ...
    , buyin     : donation + rake
    , pokertype : this._identifyPokerType(match[8])
    , limit     : this._identifyLimit(match[9])
    , level     : safeLower(match[10])
    , sb        : safeParseFloat(match[11])
    , bb        : safeParseFloat(match[12])
    , year      : safeParseInt(match[13])
    , month     : safeParseInt(match[14])
    , day       : safeParseInt(match[15])
    , hour      : safeParseInt(match[16])
    , min       : safeParseInt(match[17])
    , sec       : safeParseInt(match[18])
    , timezone  : safeUpper(match[19])
    , metadata: {
        lineno: lineno
      , raw: line
    }
